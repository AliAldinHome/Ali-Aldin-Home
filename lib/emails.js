...
function welcomeEmail(name, formType) {
  ...
  const body = `
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="border-left:2px solid ${C.gold};padding-left:16px;margin-bottom:22px;">
      ...
      <tr><td style="padding:4px 0;font-family:'Montserrat',Arial,sans-serif;font-size:13px;font-weight:300;color:${C.charcoal};">&#128172; <a href="https://wa.me/14166060494" style="color:${C.gold};text-decoration:none;">WhatsApp &mdash; fastest response</a></td></tr>
    </table>
  ...
  return { subject: `Hi ${name} &mdash; Ali Aldin has received your request`, html: shell(body, '', false) };
}
...
function welcomeFollowUp(name, token) {
  const body = `
    ...
    <p style="font-family:'Montserrat',Arial,sans-serif;font-size:14px;font-weight:300;color:${C.sub};line-height:1.85;margin:0 0 14px;">No spam, no pressure &mdash; just useful, relevant content from someone who genuinely cares about your real estate goals.</p>
  ...
  return { subject: 'A quick note from Ali &mdash; The Heartland Agent', html: shell(body, token) };
}
...
function birthdayEmail(name, token) {
  ...
  return { subject: `Happy Birthday, ${name}! &mdash; Ali Aldin`, html: shell(body, token) };
}
...
function anniversaryEmail(name, token, years) {
  ...
  const body = `
    ...
    <p style="font-family:'Montserrat',Arial,sans-serif;font-size:14px;font-weight:300;color:${C.sub};line-height:1.85;margin:0 0 14px;">It has been ${yr} since we worked together &mdash; I hope you have loved every moment in your home.</p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${C.bgAlt};border-radius:10px;border-left:3px solid ${C.gold};margin:0 0 24px;">
      <tr><td style="padding:16px 20px;font-family:'Montserrat',Arial,sans-serif;font-size:13px;font-weight:300;color:${C.charcoal};line-height:1.7;">
        Want to know what your home is worth today? <a href="https://wa.me/14166060494" style="color:${C.gold};text-decoration:none;">WhatsApp me at 416.606.0494</a> &mdash; free, no commitment.
      </td></tr>
    </table>
  ...
  return { subject: `${yr} in your home &mdash; Ali Aldin, The Heartland Agent`, html: shell(body, token) };
}
...
function marketUpdateEmail(name, token, opts) {
  ...
  const body = `
    ...
    <p style="font-family:'Montserrat',Arial,sans-serif;font-size:14px;font-weight:300;color:${C.charcoal};margin:0;">Until next quarter,<br/><strong style="font-weight:500;">Ali Aldin</strong> &mdash; The Heartland Agent</p>`;
  return { subject: subj || `GTA West Market Update &mdash; ${q}`, html: shell(body, token) };
}

// 6. ADMIN NOTIFICATION
const BADGES = {
  seller:       { e: '&#127968;', l: 'Seller &mdash; Free Valuation' },
...
function adminNotification(data) {
  ...
  const rows = [
    ...
    ['Marketing Consent', marketingConsent === 'yes' ? '&#9989; Yes &mdash; added to CRM' : '&#10060; No &mdash; inquiry only'],
    ...(notes             ? [['Notes',                notes]]                  : [])
  ].map(([l, v]) => `
    <tr>
  ...
  const subject = `${b.e} ${b.l} &mdash; ${name}${address ? ' &middot; ' + address : ''}`;
...
async function send({ to, from, subject, html }) {
  ...
  await t.sendMail({
    from: from || `"Ali Aldin &mdash; The Heartland Agent" <${process.env.GMAIL_USER}>`,
    to, subject, html
  });
}
